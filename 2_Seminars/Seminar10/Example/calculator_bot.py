...
# Функция обратного вызов точки входа в разговор

API_URL = 'https://7015.depppavlov.ai/model'


def start(update, _):
    reply_keyboard = [['Операции с рациональными числами',
                       'Настроение', 'Операции с комплексными числами', 'Cancel'],
...
def choice(update, context):
    ...
    if user_choice == 'Операции с комплексными числами':
...
def operations_rational(update, context):
    user = update.message.from_user
    logger.info('Пользователь выбрал операцию: %s: %s',
                user.first_name, update.message.text)
